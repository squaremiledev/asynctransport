...
public class Ping
{
    public static void main(String[] args)
    {
        ...
        {
            System.out.println("Usage: tcpcheck ping server|client options...");
            System.exit(1);
        }
        else if ("server".equals(args[1]))
        {
            if (args.length != 3)
            {
                System.out.println("Usage: tcpcheck ping server port");
                System.out.println("e.g.   tcpcheck ping server 8889");
                System.exit(1);
            }
        ...
        else
        {
            if (args.length != 9)
            {
                System.out.println("Usage: tcpcheck ping client remoteHost remotePort secondsWarmUp secondsRun sendingRatePerSecond respondToNth extraDataLength");
                System.out.println("e.g.   tcpcheck ping client localhost        8889            40         50               400000          100               0");
                System.exit(1);
            }
