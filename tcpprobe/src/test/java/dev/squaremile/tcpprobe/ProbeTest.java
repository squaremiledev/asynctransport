...
class ProbeTest
{
    ...
    void shouldCalculateLatencyWhenMultipleMessagesInFlight()
    {
        ...
        assertThat(probe.onTime(MILLISECONDS.toNanos(6_000), outboundBuffer, 0)).isGreaterThan(0);
        assertThat(metadata.wrap(outboundBuffer, 0).correlationId()).isEqualTo(0);
        assertThat(metadata.wrap(outboundBuffer, 0).originalTimestampNs()).isEqualTo(MILLISECONDS.toNanos(6_000));

        assertThat(probe.onTime(MILLISECONDS.toNanos(7_000), outboundBuffer, 0)).isGreaterThan(0);
        assertThat(metadata.wrap(outboundBuffer, 0).correlationId()).isEqualTo(1);
        assertThat(metadata.wrap(outboundBuffer, 0).originalTimestampNs()).isEqualTo(MILLISECONDS.toNanos(7_000));

        assertThat(probe.onTime(MILLISECONDS.toNanos(8_000), outboundBuffer, 0)).isGreaterThan(0);
        assertThat(metadata.wrap(outboundBuffer, 0).correlationId()).isEqualTo(2);
        assertThat(metadata.wrap(outboundBuffer, 0).originalTimestampNs()).isEqualTo(MILLISECONDS.toNanos(8_000));

        metadata.wrap(inboundBuffer, 0).clear().correlationId(0).originalTimestampNs(MILLISECONDS.toNanos(6_000));
        probe.onMessageReceived(inboundBuffer, 0, MILLISECONDS.toNanos(8_100));
        metadata.wrap(inboundBuffer, 0).clear().correlationId(1).originalTimestampNs(MILLISECONDS.toNanos(7_000));
        probe.onMessageReceived(inboundBuffer, 0, MILLISECONDS.toNanos(8_200));
        assertThat(probe.hasReceivedAll()).isFalse();
        metadata.wrap(inboundBuffer, 0).clear().correlationId(2).originalTimestampNs(MILLISECONDS.toNanos(8_000));
